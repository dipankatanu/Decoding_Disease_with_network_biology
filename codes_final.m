...
clear normalSampleIndices tumorSampleIndices TCGAprad geneExpressionCounts nonZeroPseudoRefSample ratios sizeFactors pseudoReferenceSample normalizedCounts
...
clear lowExpressionIndices lowExpressionData barData numLowNormal numLowTumor numLowBoth i
...
xline(1, '--r', 'Log2FC = 1', 'LineWidth', 1, 'LabelHorizontalAlignment', 'left');
xline(-1, '--b', 'Log2FC = -1', 'LineWidth', 1, 'LabelHorizontalAlignment', 'left');
...
clear upRegulatedIndices downRegulatedIndices meanTumor meanNormal foldChange pValues log2FoldChange

%% Save results
countMatrix = [geneNames, normalSamples, tumorSamples];
degMatrix = [upRegulatedGenes; downRegulatedGenes];

xlswrite("deg.xlsx", degMatrix);
...
topNodes = intersect(sortedDegreeRanks(1:10, 1), intersect(sortedClosenessRanks(1:10, 1), sortedBetweennessRanks(1:10, 1)));
